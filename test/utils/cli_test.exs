defmodule Utils.CliTest do
  # Because we are dealing with cpature_io, we run these tests synchronously so we don't capture output from other tests. (https://hexdocs.pm/ex_unit/1.12/ExUnit.CaptureIO.html)
  use ExUnit.Case
  ...
  test "enter_description/1 expects input and return updated state", %{state: state} do
    capture_io(
      ...
      fn ->
        assert Cli.enter_description(state) |> Map.get(:description) ==
                 "This is the commit description"
      end
    )
  end
